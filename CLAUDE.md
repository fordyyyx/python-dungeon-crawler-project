...
- **Intellect gating (`examine`)**: `Room.examine_text`/`required_intellect` are plain constructor data. `handle_examine(room, player)` withholds `examine_text` below the threshold, but **always still calls `reveal_hidden_exits()` regardless of Intellect** — the mechanical reveal is never gated, only the flavour text is. This is the concrete implementation of the hard rule below; don't let a future examine-adjacent feature (e.g. a locked door needing Intellect to notice) violate it.
- **Hard rule, Intellect specifically**: must never gate anything required to progress — only optional lore/discoveries. A low-Intellect ancestry pick should miss flavour text, never get soft-locked out of something needed.
