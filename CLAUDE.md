...
- One responsibility per module (current, post-reorganisation structure —
  ...
  - `combat.py` — combat resolution: `resolve_combat_round`, `resolve_attack_and_check_defeat`, `handle_enemy_defeat`, `format_hp_line`, `flee_combat`, `handle_combat_command`, `choose_enemy_action` (utility-based enemy AI — see "Enemy AI and team combat" below; decided design, not yet built)
...
- **Enemy AI fields (decided names, pending implementation — see "Enemy AI and team combat" below)**: `Enemy.aggression_weight`, `Enemy.caution_weight`, `Enemy.randomness_weight` (utility-scoring weights), `Enemy.brace_amount` (flat damage reduction from Defend), `Enemy.heal_amount` (flat HP restored by Heal, `0` for enemies with no lore-appropriate heal). All plain data set per `create_*()` call, same precedent as `experience_reward`/`gold_reward`. Do not drift from these names once built.
...
## Enemy AI and team combat — decided design (not yet built)
This is the design settled for roadmap item #1 ("Enemy `choose_action()` +
multiple enemies per room"). Nothing here is built yet — this section exists
so a future coding session implements the agreed design rather than
re-deciding it from scratch. Move the relevant parts into the sections
above once actually built, same as every other convention in this file.

- **Team representation**: no new `Team` class (see "No `Team` class" above)
  — `player_team: list[Character]` (currently `[player]`) and `enemy_team`
  (i.e. `Room._enemies`) are passed as plain lists through the combat
  functions that need to become team-aware.
- **Round order**: the player acts first (attack/item/etc., unchanged),
  then every still-living member of `enemy_team` takes a turn via
  `choose_enemy_action()`, in list order. Combat stays locked until the
  whole `enemy_team` is defeated or the player flees.
- **`choose_enemy_action(enemy, player_team)` is a standalone function in
  `combat.py`, not an `Enemy` method** — per the existing rule that
  functions coordinating two independent classes' state (an enemy's own
  state *and* the player's) live standalone, same precedent as
  `trade_with_ally()`.
- **AI model: utility-based scoring, with randomness mixed in.** Each
  candidate action gets a numeric score from a small set of weighted
  state variables (self HP%, player HP%, kill-potential, etc.); the
  highest-scoring action wins — except a random noise term is added to
  every score first, so the AI doesn't always pick optimally. This is
  the intended "lucky escape" mechanic. **Randomness must be derived from
  `random.random()`** (not `random.uniform()` or any other RNG call), to
  stay consistent with the existing monkeypatch-testable convention (see
  the monkeypatch section below, updated to include this).
- **Personality is per-enemy data, not branching** — new fields on
  `Enemy`: `aggression_weight`, `caution_weight`, `randomness_weight`
  (see "Canonical attribute names" above for the full list including
  `brace_amount`/`heal_amount`). Set per `create_*()` call, same pattern
  as `experience_reward`/`gold_reward`. Most enemies default to a
  balanced preset; named/boss enemies get bespoke values tied to lore.
- **Initial action set: Attack, Defend/Brace, Heal.** Retreat/flee is
  explicitly **player-only** — enemies never get it, full stop.
  - **Defend/Brace**: flat, per-enemy damage reduction (`brace_amount`),
    lore-tied (a bracing Cyclops deflects more than a bracing Harpy).
    Implemented as a transient "defending" flag consumed by
    `take_damage()` on the next hit — this touches `take_damage()`'s
    signature/logic, which has a documented history of bugs from careless
    changes (see above); double-check every call site.
  - **Heal**: flat HP value per enemy (`heal_amount`), lore-tied. Enemies
    with no lore-appropriate heal simply have `heal_amount = 0`, and Heal
    is excluded from their candidate action list entirely (not scored at
    zero — excluded).
  - **Deliberately extensible, not extended yet**: Light/Heavy/
    Ranged/Spell attack variety (roadmap #8) and mana-based Spell-healing
    (roadmap #7) become *additional candidate actions* in the same
    scoring function once those items are built — this is not a redesign
    of the AI, just more entries in the list `choose_enemy_action()`
    already scores.
- **`format_hp_line` generalizes** from its current 2-combatant signature
  to an N-combatant display. Still exactly one shared helper — every call
  site gets re-checked when this lands, per its own documented history of
  drifting apart once already.
- **Flee stays per-enemy**: with a full enemy team, every still-living
  enemy independently rolls its own parting-hit chance off its own HP% —
  same existing math as today, just applied per member instead of once
  for a single enemy.
- **New player command: `target <name>`** — sets the current attack
  target; persists across rounds, so a bare `attack` afterward hits
  whatever is currently targeted. **Disambiguation**: when a room has two
  or more enemies sharing a name, each gets a stable index based on its
  position in `Room._enemies` (shown in combat displays — the HP line,
  etc. — only when a name is actually ambiguous in that room; unique
  names show no number, unchanged from today). Targeted via
  `target <name> <number>`, e.g. `target harpies 2`.
- **Documentation sync required once built**: `target` and the
  disambiguation numbering need adding to both `get_controls_text()` and
  the README's Controls section, per the doc-sync reminder below — this
  is a new command, not just an internal change.
...
- **`get_controls_text()` (engine.py) and the README's Controls section
  ...
  (This will apply to the new `target` command once Enemy AI/team combat
  is built — see above.)
...
1. **Controlling randomness** — anything using `random` (currently
   ...
   Heavy Attack's miss chance once built, and to
   `choose_enemy_action()`'s utility-score randomness term once Enemy
   AI is built — see "Enemy AI and team combat" above) should use
